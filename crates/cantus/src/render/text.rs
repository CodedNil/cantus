...
/// Fixed text display list embedded directly in a pass's ordinary state.
#[isthmus::data]
...
#[isthmus::outline]
// `f32::clamp` has a panic path, which forces Rust-GPU to inline this function.
#[allow(clippy::manual_clamp)]
fn edge_distance(edge: Edge, weight: f32, point: Vec2) -> (f32, i32) {
    let a = edge.low_start + (edge.high_start - edge.low_start) * weight;
    let control = edge.low_control + (edge.high_control - edge.low_control) * weight;
    let b = edge.low_end + (edge.high_end - edge.low_end) * weight;
    let chord = b - a;
    ...
    while iteration < 3 {
        ...
        let denominator = if denominator.abs() < 1e-8 {
            if denominator < 0.0 { -1e-8 } else { 1e-8 }
        } else {
            denominator
        };
...
mod cpu {
    use super::{
        EFFECT_PADDING, Edge, Glyph, Line, MAX_LINE_GLYPHS, PlacedGlyph, TextStyle, Vec2, vec2,
    };
    ...
    impl Font {
        pub fn new(passes: &Passes<'_>) -> Self {
            ...
            let glyphs =
                passes.storage("Vector Font Glyphs", metadata.iter().map(|(_, meta)| meta.data));
            Self {
                characters,
                baseline,
                edges,
    ...
    impl<const LINES: usize, const GLYPHS: usize> super::Text<LINES, GLYPHS> {
