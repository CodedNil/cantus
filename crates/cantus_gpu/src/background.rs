...
use crate::common::{mix_f32, mix_vec3, sd_squircle, smoothstep, step, unpack4x8unorm};
...
pub fn fs_background(
    ...
    let mut color = mix_vec3(
        mix_vec3(c0, c1, mix_val),
        mix_vec3(c3, c2, s2 * 0.5 + 0.5),
        mix_val,
    );
    color = mix_vec3(color, (c0 + c1 + c2 + c3) * 0.25, 0.1);
    let luma = color.dot(vec3(0.2126, 0.7152, 0.0722));
    color = mix_vec3(
        vec3(luma, luma, luma),
        color,
        mix_f32(3.2, 1.6, smoothstep(0.1, 0.4, luma)),
    );
    ...
    let tex = images.sample(*sampler, uv_img.extend(pill.image_index.max(0) as f32));
    let img_mask =
        (1.0 - smoothstep(
            -0.5,
            0.5,
        ...
        )) * step(0.0, pill.image_index as f32)
            * step(img_x, local_x);
    color = mix_vec3(color, tex.truncate(), img_mask * tex.w);
    ...
    color += mix_vec3(color, vec3(1.0, 1.0, 1.0), 0.3) * (sheen + rim + mouse_sheen);
    ...
    *out_color = vec4(
        color.x * mask * pill.alpha,
        color.y * mask * pill.alpha,
        color.z * mask * pill.alpha,
        mask.max(shadow) * pill.alpha,
    );
}
