use crate::{
    direction_and_length, pill_fragment, pixel_to_ndc, quad_coord, sd_capsule_box, sd_star,
    smooth_union, unpack3x8unorm,
};
use cantus_shared::{
    BACKPLATE_RADIUS, GlobalUniforms, ICON_WIDTH, MAX_PILL_PLAYLIST_ICONS, TrackPill, smoothstep,
};
...
pub fn fs_track(
    ...
    let (ripple_dir, ripple_strength, ripple_flash) = if (-0.02..1.02).contains(&anim_t) {
        ...
        let ripple_flash = ripple_decay * ripple_wave * 0.5;
        (ripple_dir, ripple_decay * ripple_flash, ripple_flash)
    } else {
        (Vec2::ZERO, 0.0, 0.0)
    };
    ...
    let fields = [
        (vec2(2.1, 0.7), 1.0, 0.0),
        (vec2(0.6, -2.4), -0.8, seed + FRAC_PI_2),
        (vec2(-1.5, 1.9), 0.65, 2.0),
        (vec2(2.4, 1.6), -0.55, seed + FRAC_PI_2),
    ];
    let mut color = Vec3::ZERO;
    let mut weight_sum = 0.0;
    for index in 0..4 {
        let packed = pill.colors[index];
        let (direction, speed, offset) = fields[index];
        let wave = (warped_uv.dot(direction) + flow_time * speed + offset).sin() * 0.5 + 0.5;
    ...
    let twinkle =
        (global.time * (0.7 + phase * 0.9 + audio.acousticness * 0.8) + phase * TAU).sin() * 0.5
            + 0.5;
    let speck = smoothstep(0.985 - audio.acousticness * 0.09, 1.0, hash(cell, seed))
    ...
    let sheen = smoothstep(0.12, 0.0, stretched_uv_y) * mask * 0.13
        + smoothstep(5.0, -3.0, dist) * 0.08
        + smoothstep(30.0, 0.0, (mouse_d - 15.0).abs()) * mouse_inf * 0.18;
    color += color.lerp(Vec3::ONE, 0.32) * sheen;
