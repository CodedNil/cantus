...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    let Data::Struct(data) = &input.data else {
        return syn::Error::new_spanned(input, "Render requires a struct").to_compile_error().into();
    };
    let Fields::Named(fields) = &data.fields else {
        return syn::Error::new_spanned(input, "Render requires named fields").to_compile_error().into();
    };
    let fields = fields.named.iter().filter_map(|field| {
        let skipped = field
            .attrs
            .iter()
            .any(|attribute| attribute.path().is_ident("render") && attribute.parse_args::<Ident>().is_ok_and(|value| value == "skip"));
        (!skipped).then(|| field.ident.as_ref().unwrap())
    });
    implementation(name, &input.generics, fields).into()
}

pub fn implementation(name: &Ident, generics: &Generics, fields: impl IntoIterator<Item = impl ToTokens>) -> TokenStream2 {
