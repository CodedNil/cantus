...
impl StageContract {
    pub fn parse(function: &ItemFn, stage: Stage) -> SynResult<Self> {
        ...
        for (argument_index, argument) in function.sig.inputs.iter().enumerate() {
            ...
            let Some(attribute) = attribute else {
                return Err(syn::Error::new_spanned(
                    argument,
                    "shader parameters require a `#[gpu(...)]` role",
                ));
            };
            ...
            if matches!(role.role, GpuRole::VertexIndex | GpuRole::InstanceIndex)
                && !is_named_type(&argument.ty, "u32")
            {
                return Err(syn::Error::new_spanned(
                    &argument.ty,
                    "GPU index parameters must use `u32`",
                ));
            }
            let Pat::Ident(pattern) = argument.pat.as_ref() else {
                return Err(syn::Error::new_spanned(
                    &argument.pat,
                    "GPU parameters require an identifier",
                ));
            };
            if role_is_unique(role.role)
                && parameters
                    .iter()
                    .any(|parameter: &StageParameter| parameter.role.role == role.role)
            {
                return Err(syn::Error::new_spanned(
                    &argument.attrs[attribute],
                    "a shader stage can only declare this GPU role once",
                ));
            }
    ...
    pub fn has_implicit(&self, role: GpuRole) -> bool {
        self.parameters
            .iter()
            .any(|parameter| parameter.role.role == role && parameter.role.index.is_none())
    }
}
...
fn validate_role(parameter: &GpuParameter, stage: Stage) -> SynResult<()> {
    if parameter.index.is_some() && parameter.role != GpuRole::Instance {
        return Err(syn::Error::new(
            parameter.kind.span(),
            "only instance parameters can specify an index",
        ));
    }
    let allowed = match stage {
        ...
        Stage::Fragment => matches!(
            parameter.role,
            GpuRole::Shared | GpuRole::Instance | GpuRole::Resource
        ),
    };
    if allowed {
        Ok(())
        ...
        Err(syn::Error::new(
            parameter.kind.span(),
            "this GPU role is not valid for this shader stage",
        ))
    }
}
...
pub fn gpu_attribute(attributes: &[Attribute]) -> SynResult<Option<usize>> {
    let mut matches = attributes
        .iter()
        .enumerate()
        .filter(|(_, attribute)| attribute.path().is_ident("gpu"));
    let found = matches.next().map(|(index, _)| index);
    if let Some((_, duplicate)) = matches.next() {
        return Err(syn::Error::new_spanned(
            duplicate,
            "only one GPU role may be specified",
        ));
    }
    Ok(found)
}
