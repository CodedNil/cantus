...
pub fn derive(input: TokenStream) -> TokenStream {
    ...
    let Fields::Named(_) = &item.fields else {
        return syn::Error::new_spanned(item, "Varyings requires named fields").to_compile_error().into();
    };
    ...
    if let Fields::Named(fields) = &mut item.fields {
        for field in &mut fields.named {
            ...
            if path.qself.is_none()
                ...
                && matches!(path.path.segments[0].ident.to_string().as_str(), "Vec2" | "Vec3" | "Vec4" | "UVec2" | "UVec3" | "UVec4")
            {
